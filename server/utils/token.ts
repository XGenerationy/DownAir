...
function keyBuffer(): Buffer {
  const hex = requireSecrets().encryption;
  if (hex.length < 64) {
...
export function createDownloadToken(
  ...
  const { hmac: hmacSecret } = requireSecrets();
  const expiresAt = Date.now() + TOKEN_EXPIRY_MINUTES * 60 * 1000;
  const iv = crypto.randomBytes(16).toString('hex');

  const payload = JSON.stringify({ downloadId, sourceUrl, format, expiresAt });

  const cipher = crypto.createCipheriv(ALGORITHM, keyBuffer(), Buffer.from(iv, 'hex'));
  ...
  return Buffer.from(JSON.stringify({ encrypted, iv, authTag, hmac })).toString('base64url');
}
...
export function verifyDownloadToken(token: string): TokenData | null {
  try {
    ...
    const decipher = crypto.createDecipheriv(ALGORITHM, keyBuffer(), Buffer.from(decoded.iv, 'hex'));
...
export function generateSecrets(): { jwtSecret: string; encryptionKey: string; hmacSecret: string } {
  return {
